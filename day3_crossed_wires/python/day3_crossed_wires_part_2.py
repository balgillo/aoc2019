...
class Move:
    ...
    def apply(self, x0, y0, s0):
        if self.direction == Direction.up:
            return (x0, y0 - self.distance, s0 + self.distance)
        elif self.direction == Direction.down:
            return (x0, y0 + self.distance, s0 + self.distance)
        elif self.direction == Direction.left:
            return (x0 - self.distance, y0, s0 + self.distance)
...
def find_closest_crossing(wire1_moves, wire2_moves):
    ...
    for wire1_move in wire1_moves:
        ...
        for wire2_move in wire2_moves:
            ...
            if c:
                (cx, cy) = c
                c_steps = wire1_steps + abs(cx - wire1_x) + abs(cy - wire1_y) + wire2_steps + abs(cx - wire2_x) + abs(cy - wire2_y)
                best_crossing_distance = closest(best_crossing_distance, c_steps)
            (wire2_x, wire2_y, wire2_steps) = wire2_move.apply(wire2_x, wire2_y, wire2_steps)
        (wire1_x, wire1_y, wire1_steps) = wire1_move.apply(wire1_x, wire1_y, wire1_steps)
